...
export default function Background({ theme = 'dark' }) {
  ...
  const blobOpacity = light ? 0.35 : 1
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden bg-ink-900">
      ...
      <div
        ...
        style={{
          background: light
            ? 'radial-gradient(120% 120% at 20% 0%, #f3f0fb 0%, #eef0f7 45%, #e7e9f4 100%)'
      ...
      <motion.div
        ...
        style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.55), transparent 60%)', opacity: blobOpacity }}
        animate={{ x: [0, 80, -40, 0], y: [0, -60, 40, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute top-1/3 -right-1/4 h-[60vh] w-[60vh] rounded-full blur-[130px]"
        style={{ background: 'radial-gradient(circle, rgba(56,189,248,0.35), transparent 60%)', opacity: blobOpacity }}
        animate={{ x: [0, -70, 30, 0], y: [0, 50, -30, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-0 left-1/3 h-[55vh] w-[55vh] rounded-full blur-[120px]"
        style={{ background: 'radial-gradient(circle, rgba(16,185,129,0.28), transparent 60%)', opacity: blobOpacity }}
